...
const apiFilePath = path.join(__dirname, 'app/api/enrollments/[id]/route.ts');
const currentContent = fs.readFileSync(apiFilePath, 'utf8');

// Create the new content with proper payment handling
...
export async function PUT(
  ...
  const { id } = params
  const { totalAmount, paidAmount, status, startDate } = await request.json()
...
console.log('✅ Enrollment API updated successfully!');
console.log('📝 Changes made:');
console.log('   - Added payment record creation when paid amount increases');
console.log('   - Added payment record adjustment when paid amount decreases');
console.log('   - Used database transaction to ensure data consistency');
console.log('   - Fixed paymentDate field reference to use paidAt');
console.log('');
